...
public class TDSCalculatorClient extends RestClient
{
	        ...
	        final DateTime creditDate) throws SandboxException
	{
		...
		if (deducteeType != null && !deducteeType.isEmpty() && isPanAvailable != null && residentialStatus != null
		        && !residentialStatus.isEmpty() && section != null && !section.isEmpty() && creditAmount != null
		        && creditDate != null)
		{
			...
			request.put("credit_date", DateTimeFormat.forPattern("dd/MM/yyyy")
			        .withZone(DateTimeZone.forID("Asia/Kolkata")).print(creditDate));
